...
- Codex CLI: user/project-scope install. The root `AGENTS.md` 480ai managed block provides the architect main prompt, and the custom agent set contains only the four subagents. Review runs are normally parallel with `480-code-reviewer` and `480-code-reviewer2`, and the rest of the delegation budget stays narrow. Install merges `features.multi_agent = true`, `agents.max_depth = 2`, and optional desktop notifications into `config.toml`. Install state and no-op delegation validation are available through `python3 -m app.manage_agents verify --target codex --scope user`.
