...
- Codex CLI: user / project 범위 설치, 루트 `AGENTS.md` 480ai 관리 블록이 architect 메인 프롬프트를 맡고 custom agent는 4개 서브에이전트만 설치. reviewer 흐름은 기본적으로 `480-code-reviewer`, `480-code-reviewer2` 병렬이며 그 외 delegation budget은 좁게 유지합니다. 설치 시 `config.toml`에는 `features.multi_agent = true`, `agents.max_depth = 2`만 최소 merge
