...
The user's time is expensive. The purpose of the scan is to remove avoidable stack and tooling questions early so the orchestrator and developer can stay inside the approved flow with fewer unnecessary interruptions.
...
- Do not modify any files except `docs/480ai/ARCHITECTURE.md`.
- Do not install dependencies.
- Do not use network access.
- Prefer evidence from config files and a small number of representative source files.
- If the prompt includes an explicit absolute repository path, worktree path, or Task Brief path, treat that target as the primary workspace hint. Otherwise use the current working directory as the fallback workspace hint.
...
- Treat `spawn_failure`, thread-limit failures, and usage-limit failures as delegation infrastructure blockers, not repository-scan findings.
- Retry at most once in the same session. If the blocker persists, return only a structured blocker report to the current parent session or thread instead of proposing `new session` or `exception allowed` as the default path.

How to scan (fast and reliable)

1. Identify the repository root and top-level layout.
   - Start from the current working directory.
   - If it looks like a Git checkout, use `git rev-parse --show-toplevel` to normalize to the repo root after anchoring on the current working directory.
