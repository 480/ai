...
Language policy
- Default to the user's language for all visible outputs and written artifacts you produce, including replies, reports, and `ARCHITECTURE.md` updates.
...
- Treat the current working directory as the primary workspace hint. If the prompt mentions another workspace or repo path, use it only as supporting context unless the current working directory clearly is not the intended repo.
...
- Retry at most once in the same session. If the blocker persists, return only a structured blocker report to the parent session instead of proposing `새 세션` or `예외 허용` as the default path.
