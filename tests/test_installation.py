...
class InstallationTests(unittest.TestCase):
    ...
    def assert_summary_contains_provider(
        ...
        *,
        ...
        if activate_default is not None:
            self.assertIn(f"기본 활성화: {'yes' if activate_default else 'no'}", summary)
        else:
            self.assertNotIn("기본 활성화:", summary)
        self.assertIn(f"모델 모드: {model_mode}", summary)
    ...
    def assert_claude_team_contract(self, text: str, *, named_team_members: bool = True) -> None:
        ...
        self.assertRegex(text, r"(agent team|팀)")
        ...
        self.assertRegex(text, r"(only when|필요할 때만|선택적으로)")
        self.assertRegex(text, r"(fallback|single-orchestrator)")
        self.assertRegex(text, r"(disabled|unsupported|미지원|비활성화)")
    ...
    def assert_claude_teams_install_contract(
        ...
        *,
        ...
        self.assertRegex(text, r"(설치|install)")
        self.assertRegex(text, r"(agent teams|teams.*실험 플래그|실험 플래그)")
        self.assertRegex(text, r"(묻|질문|물어보|활성화 여부)")
        ...
        self.assertRegex(text, r"(merge|병합|반영)")
        if mention_uninstall:
            self.assertRegex(text, r"(제거|uninstall)")
            self.assertRegex(text, r"(건드리지 않|유지|변경하지 않)")
    ...
    def assert_claude_teams_prompt_shown(self, text: str) -> None:
        ...
        self.assertRegex(text, r"실험 플래그")
    ...
    def test_claude_legacy_invalid_backup_path_still_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
    ...
    def test_claude_non_string_legacy_backup_entry_still_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            _config_path, state_path = self.seed_legacy_claude_install(home)
            state = self.read_json(state_path)
    ...
    def test_prompt_install_options_blocks_unsupported_opencode_project_and_allows_opt_out(self) -> None:
        ...
        self.assertIn("project - 지원하지 않음", output)
        self.assertIn("project 범위는 이 대상에서 아직 지원하지 않습니다.", output)
        self.assertIn("기본 에이전트를 바로 활성화할까요?", output)
        self.assertIn("1) 예", output)
        self.assertIn("2) 아니오", output)
        self.assertNotIn("agent teams 실험 플래그", output)
        self.assertIn("모델 모드를 선택하세요.", output)
    ...
    def test_prompt_install_options_hides_activation_for_codex(self) -> None:
        ...
        self.assertIsNone(request.model_selection)
        self.assertNotIn("기본 에이전트를 바로 활성화할까요?", stdout.getvalue())
        self.assertNotIn("agent teams 실험 플래그", stdout.getvalue())
    ...
    def test_prompt_install_options_collects_advanced_role_model_choices(self) -> None:
        ...
        output = stdout.getvalue()
        self.assertIn("고급 모드: role별 curated 모델을 선택하세요.", output)
        self.assertIn("480-architect 모델을 선택하세요.", output)
        self.assertIn("480-code-scanner 모델을 선택하세요.", output)
    ...
    def test_prompt_install_options_tui_asks_each_provider_in_order_and_renders_summary(self) -> None:
        ...
        self.assert_screen_title_contains(ordered_titles, "OpenCode", "범위")
        self.assert_screen_title_contains(ordered_titles, "OpenCode", "활성화")
        self.assert_screen_title_contains(ordered_titles, "OpenCode", "모델 모드")
        self.assert_screen_title_contains(ordered_titles, "Claude Code", "범위")
        self.assert_screen_title_contains(ordered_titles, "Claude Code", "활성화")
        self.assert_screen_title_contains(ordered_titles, "Claude Code", "agent teams")
        self.assert_screen_title_contains(ordered_titles, "Claude Code", "모델 모드")
        for role in CLAUDE_AGENTS:
            self.assert_screen_title_contains(ordered_titles, "Claude Code", role, "모델")

        summary_title, summary_lines, _summary_footer = rendered_screens[-1]
        self.assertIn("요약", summary_title)
    ...
    def test_detected_provider_choices_filters_by_cli_binary_and_keeps_config_dir_as_note(self) -> None:
        ...
        self.assertIn("설정 디렉터리 감지", choices[0].note)
    ...
    def test_repeated_install_then_uninstall_preserves_original_backup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual((agents_dir / "480-architect.md").read_text(encoding="utf-8"), original_architect)
            ...
            (agents_dir / "480-architect.md").unlink()
            self.run_command(home, "uninstall")

            self.assertEqual((agents_dir / "480-architect.md").read_text(encoding="utf-8"), original_architect)
            self.assertFalse(state_path.exists())
            self.assertFalse(backup_path.exists())

    def test_install_rejects_invalid_managed_agents_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            config_path = home / ".config" / "opencode" / "opencode.json"
            state_path = home / ".config" / "opencode" / ".480ai-bootstrap" / "state.json"
    ...
    def test_retry_after_failed_final_install_state_write_fails_before_fake_backup_creation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(backup_path.exists())
            self.assertEqual(architect_path.read_text(encoding="utf-8"), architect_source)
            self.assertEqual(self.read_json(state_path)["managed_file_metadata"]["480-architect"], None)
    ...
    def test_partial_uninstall_then_reinstall_and_uninstall_preserves_live_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("live file was modified and backup exists", install_result.stderr)
            self.assertEqual(uninstall_result.returncode, 0)
    ...
    def test_remote_bootstrap_scripts_default_to_public_repo_without_auth(self) -> None:
        ...
        install_section = self.readme_section("설치", readme)
        uninstall_section = self.readme_section("삭제", readme)

        self.assertIn("sh -c", install_section)
        self.assertIn("curl", install_section)
        self.assertIn("install-remote.sh", install_section)
        self.assertNotIn("| sh", install_section)
        ...
        self.assertIn("| sh", uninstall_section)
        self.assertIn("uninstall-remote.sh", uninstall_section)
        self.assertNotRegex(uninstall_section, r"(?<![A-Za-z-])(?:bootstrap/)?install-remote\.sh")
        self.assertNotIn("sh -c", uninstall_section)
    ...
    def test_readme_covers_slim_bootstrap_flow_and_core_references(self) -> None:
        ...
        self.assertIn("## 삭제", readme)
        self.assertRegex(install_section, r"여러\s+provider")

        supported_targets = self.readme_section_any(("지원 대상", "Providers"), readme)
        ...
        self.assertNotRegex(codex_line.group(0), r"480-architect.*활성화|활성화.*480-architect")
    ...
    def test_opencode_index_and_architecture_docs_match_current_bootstrap_behavior(self) -> None:
        ...
        self.assertIn("기본값으로 `480-architect`를 활성화", opencode_index)
        self.assertIn("`--no-activate-default` 또는 `BOOTSTRAP_ACTIVATE_DEFAULT=0`", opencode_index)
        self.assertIn("현재 설정이 아직 `480-architect`일 때만", opencode_index)
        self.assertIn("`고급` 설치는 선택한 모델 조합으로 임시 산출물을 렌더링", opencode_index)
        ...
        self.assertIn("Codex는 루트 `AGENTS.md`의 480ai 관리 블록을 architect 메인 프롬프트로 사용합니다.", codex_index)
        self.assertIn("`providers/codex/instructions/480-architect.md`", codex_index)
        self.assertIn("architect custom agent는 따로 만들지 않습니다.", codex_index)
        self.assertIn("Codex custom agent는 아래 4개 서브에이전트만 제공합니다.", codex_index)
        ...
        self.assertIn("reviewer 기본 흐름은 병렬입니다", codex_index)
        ...
        expected_short_signoff_contract = (
            ...
            "(for example, Korean `승인`, English `approved`)."
        )
        for architect_doc in (opencode_architect, claude_architect, codex_managed_guidance):
            ...
            self.assertNotIn("계획 승인:", architect_doc)
            self.assertNotIn("요구사항 승인", architect_doc)
        ...
        self.assertIn("request review from `480-code-reviewer` and `480-code-reviewer2` in parallel", codex_developer["developer_instructions"])
        ...
        self.assertIn("Expect `480-code-reviewer2` to review the same task in parallel", codex_reviewer["developer_instructions"])
        ...
        self.assertIn("Expect `480-code-reviewer` to review the same task in parallel", codex_reviewer2["developer_instructions"])
