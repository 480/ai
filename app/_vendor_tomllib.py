...
class TOMLDecodeError(ValueError):
    """Raised when parsing an invalid TOML document."""
...
class Flags:
    ...
    def unset_all(self, key: Key) -> None:
        cont = self._flags
        for current in key[:-1]:
            if current not in cont:
                return
            cont = cont[current]["nested"]
        cont.pop(key[-1], None)

    def set(self, key: Key, flag: int, *, recursive: bool) -> None:  # noqa: A003
        cont = self._flags
        key_parent, key_stem = key[:-1], key[-1]
        for current in key_parent:
            if current not in cont:
                cont[current] = {"flags": set(), "recursive_flags": set(), "nested": {}}
            cont = cont[current]["nested"]
...
def key_value_rule(src: str, pos: Pos, out: Output, header: Key, parse_float: ParseFloat) -> Pos:
    ...
    relative_path_cont_keys = (header + key[:index] for index in range(1, len(key)))
...
def parse_value(src: str, pos: Pos, parse_float: ParseFloat) -> tuple[Pos, Any]:  # noqa: C901
    ...
    if datetime_match:
        try:
            ...
            raise suffixed_err(src, pos, "Invalid date or datetime") from exc
