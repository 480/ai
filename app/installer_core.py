...
def has_live_backup(target: InstallTarget, state: dict, name: str) -> bool:
...
def load_state(target: InstallTarget, agent_names: list[str]) -> dict:
    if not target.paths.state_file.exists():
        return default_state(agent_names)
    state = migrate_legacy_state(target, read_json_object(target.paths.state_file), agent_names)
...
def install(
    ...
    for name in agent_names:
        ...
        live_backup = state_backup_path(target, state, name)
        backup_bytes = read_bytes(live_backup) if live_backup is not None else None

        if is_ambiguous_interrupted_install_retry(target, state, name, current_bytes, desired_bytes, force):
            raise SystemExit(
        ...
        if (
            ...
            and has_live_backup(target, state, name)
        ...
        if current_bytes is not None and (
            not is_safe_managed_install_target(target, state, name, current_bytes, desired_bytes, force)
            or (name in state["backups"] and not has_live_backup(target, state, name))
        ):
            if (
                not force
            ...
            if not (
                ...
                and current_bytes == desired_bytes
                and not has_live_backup(target, state, name)
            ):
                if not has_live_backup(target, state, name):
...
def uninstall(target: InstallTarget, source_agents_dir: Path, agent_names: list[str]) -> None:
    ...
    for name in state.get("managed_agents", agent_names):
        ...
        if backup_relative:
            ...
            if destination.exists() and should_remove_without_backup(
                source_agents_dir, target, name, current_bytes
            ):
                state["managed"][name] = False
                state["managed_file_metadata"][name] = None
                state["pending_cleanup"][name] = True
    ...
    for name in state.get("managed_agents", agent_names):
        ...
        ensure_not_symlink(destination)
        backup_relative = state.get("backups", {}).get(name)
        expected_bytes = expected_managed_bytes(source_agents_dir, target, name)
        current_bytes = read_bytes(destination)
        ...
        if backup_relative:
            ...
            break
        if state["pending_cleanup"][name] and destination.exists() and current_bytes != expected_bytes:
            cleanup_deferred = True
            break
    ...
    if not cleanup_deferred:
        ...
        if target.paths.backup_dir.exists() and not any(target.paths.backup_dir.iterdir()):
